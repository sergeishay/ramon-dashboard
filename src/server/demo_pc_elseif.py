...
class RamonBE:
    ...
    def execute_command(self, function_name, *args):
        self.setup_connections()
        if function_name == "get_device_status":
            cmd_type = GET_DEVICE_STATUS
        elif function_name == "get_files_status":
            cmd_type = GET_FILES_STATUS
        elif function_name == "upload_file_to_stream" and args:
            cmd_type = UPLOAD_FILE_TO_STREAM
            data = args[0]
        # Add other commands as needed
        else:
            print(f"Invalid command: {function_name}")
            self.close_connections()
            return

        message = self.pack(cmd_type, data if 'data' in locals() else "")
        self.send_all(self.requests_socket, message)
        response = self.receive_all(self.responses_socket)
        self.close_connections()
        return response
...
if __name__ == '__main__':
    if len(sys.argv) > 1:
        ...
        args = sys.argv[2:]
        response = demo_instance.execute_command(function_name, *args)
